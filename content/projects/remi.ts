...
const project: Project = {
  slug: 'remi',
  name: 'Remi',
  kicker: 'CASE STUDY · AI COMPANION',
  oneLiner: '跨终端、持续在线的实时 AI 陪伴系统——低延迟语音双工、可打断、有长期记忆的 AI 伴侣。',
...
这个定位决定了工程难点不在单次回复质量，而在三件事：**低延迟实时交互**、**跨会话关系记忆**、**多终端一致的存在感**。整个架构都围绕它们展开。`,
  blocks: [
    {
      type: 'prose',
      heading: '双脑架构：Fast Brain + Slow Brain',
      markdown: `实时陪伴有一对天然矛盾：回复要快（否则对话节奏断裂），理解要深（否则记不住关系）。Remi 用双脑架构拆开它：
...
两条路径由上下文编排层统一调度：情绪状态、记忆召回、快慢脑产出在这里汇合成最终 prompt。用户感知到的是「她反应很快，而且记得我们之前聊过什么」。`,
    ...
    {
      type: 'prose',
      heading: '语音全链路：STT → VAD → LLM → TTS',
      markdown: `Remi 的语音不是「录一段、传一段」，而是完整的实时双工链路：
...
全链路配有延迟追踪器，固定输出 speech_end → stt_final → llm → tts → playback 各阶段指标，优化有前后版本可比。`,
    ...
    {
      type: 'prose',
      heading: '长期记忆：让关系可以积累',
      markdown: `记忆系统分层演进，当前是以 episode 为核心的 V2 架构：
...
- **会话 overlay**：会话启动时从持久层预加载少量事实型记忆到本地副本，实时链路只读本地副本、写回异步，保证记忆不拖慢实时对话。`,
    ...
    {
      type: 'prose',
      heading: '四端架构：一套协议，四种形态',
      markdown: `Remi 覆盖 Web（Next.js）、桌面（Tauri v2 透明悬浮窗 + 聊天面板双窗口）、iOS（原生 Swift，语音双工 + Live2D 骨架）、watchOS（情绪手环：表情脸 + PTT 语音 + HealthKit 主动关怀 + 表盘 Complication）四个终端，共享同一套 WebSocket 协议，服务端按客户端类型自动协商能力。
...
| 基础设施 | Clerk / JWT 认证、pino 结构化日志 + 延迟追踪、Docker Compose 部署 |`,
    },
    {
      type: 'prose',
      heading: '项目状态',
      markdown: `实时语音与打断体验处于 Beta 前期，记忆 V2 主路径已接通并在质量观察期；架构为插件系统、直播/游戏接入与穿戴设备预留了扩展边界。`,
    },
  ],
}
